...
REARM_SECONDS="${OKF_WIKI_STOP_REARM_SECONDS:-21600}"   # 6h between nudges
REARM_EDITS="${OKF_WIKI_STOP_REARM_EDITS:-10}"          # new edits required
SESSION_ID=$(printf '%s' "$INPUT" | python3 -c "
import json, sys; print(json.load(sys.stdin).get('session_id', ''))" 2>/dev/null || echo "")
SENTINEL=""
REARM_CANDIDATE=0
if [[ -n "$SESSION_ID" ]]; then
  SENTINEL="${TMPDIR:-/tmp}/okf-wiki-stop-capture-${SESSION_ID}.done"
  if [[ -e "$SENTINEL" ]]; then
    NOW=$(date +%s)
...
# cannot run /wiki-capture (which writes to the bundle) even if nudged. Skip
...
CLASSIFIER="${TMPDIR:-/tmp}/okf-wiki-stop-capture-classify-$$.py"

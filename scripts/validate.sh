#!/usr/bin/env bash
# okf-wiki Bundle Validator (OKF v0.2 + okf-wiki out-of-scope convention)
...
#
# okf-wiki convention (PRD §6.6): out-of-scope zones are excluded from
# validation — `_`-prefixed directories and files, dot-files and dot-directories.
#
# Exit codes: 0 = conformant; 1 = at least one E-class error; 2 = usage error.
...
if [ $# -lt 1 ]; then
  echo -e "${RED}Usage: $0 <bundle-path>${NC}" >&2
  exit 2
fi
...
if [ ! -d "$BUNDLE" ]; then
  echo -e "${RED}Error: '$BUNDLE' is not a directory${NC}" >&2
  exit 2
fi

# `cd` into the bundle so that the `-path` patterns below match against
# bundle-relative paths only. Matching against the absolute path would let
# `_` or `.` segments in the parent chain (e.g. /home/user/_projects/...)
# silently exclude the whole bundle.
cd "$BUNDLE" || exit 2
...
echo "Convention: okf-wiki out-of-scope zones excluded from validation"
echo "---"

# Get current timestamp for staleness check
NOW=$(date -u +%Y-%m-%dT%H:%M:%SZ)

# Find all in-scope .md files (okf-wiki convention: `_`-prefixed dirs/files
# and dot-files/dirs are out of scope and excluded).
...
done < <(find . -name "*.md" -type f -not -path "*/.*" -not -path "*/_*/**" -not -name "_*" -not -name ".*" -print0 | sort -z)

# Count .md files skipped as out of scope (informational)
while IFS= read -r -d '' skipped; do
  SKIPPED_OUT_OF_SCOPE=$((SKIPPED_OUT_OF_SCOPE + 1))
done < <(find . -name "*.md" -type f \( -path "*/.*" -o -path "*/_*/**" -o -name "_*" -o -name ".*" \) -print0 | sort -z)
...
echo "Skipped (out of scope): $SKIPPED_OUT_OF_SCOPE"

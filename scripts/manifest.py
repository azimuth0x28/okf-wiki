#!/usr/bin/env python3
"""Manifest helper for the okf-wiki bundle — normalize paths and compute ingest deltas.
...
Usage:
  ...
  python3 scripts/manifest.py normalize <bundle_path> [--dry-run]
  ...
  python3 scripts/manifest.py delta <bundle_path> --scan '<glob>' [--skip a,b]
"""
...
def cmd_normalize(args: argparse.Namespace) -> int:
    ...
    for key, entry in sources.items():
        if not os.path.isabs(key):
            # Real bundles store some keys relative to the ingest root (e.g.
...
def _relative_key_index(sources: dict) -> dict[str, list[tuple[str, dict]]]:
    ...
    Real bundles store many keys relative to the ingest root (e.g.
    "-Users-x-github/abc.jsonl" under ~/.claude/projects/). canonical()
...
def main(argv: list[str] | None = None) -> int:
    ...
    n.add_argument("vault", help="path to the okf-wiki bundle (contains .manifest.json)")
    ...
    d.add_argument("vault", help="path to the okf-wiki bundle")

...
description: >
  Ingest OpenClaw agent history into the OKF knowledge bundle. Use this skill when the user wants to mine
  ...
  sessions to the bundle", "ingest ~/.openclaw", or "what have I worked on in OpenClaw". Also triggers
...
You are extracting knowledge from the user's OpenClaw agent history and distilling it into the OKF knowledge bundle. OpenClaw stores both a structured long-term MEMORY.md and per-session JSONL transcripts — focus on durable knowledge, not operational telemetry.
...
Read it in full and extract concept-level knowledge. Do not create one bundle page per MEMORY.md entry — cluster by topic.
...
Do not create one bundle page per session or per MEMORY.md entry.
...
## Step 6: Distill into Bundle Pages
...
- Record provenance in frontmatter on every new/updated page:
  ...
  - `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:openclaw-history/<session-id>` (one entry per memory file, daily note, or session the knowledge was distilled from).
...
Derived from Ar9av/obsidian-wiki openclaw-history-ingest skill (MIT).

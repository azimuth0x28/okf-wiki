...
description: >
  Ingest Codex CLI conversation history into the OKF knowledge bundle. Use this skill when the user wants to mine
  their past Codex sessions for knowledge, import their ~/.codex folder, extract insights from previous coding
  sessions, or says things like "process my Codex history", "add my Codex conversations to the bundle", or
...
You are extracting knowledge from the user's past Codex sessions and distilling it into the OKF knowledge bundle. Session logs are rich but noisy: focus on durable knowledge, not operational telemetry.
...
For each impacted project, create/update `projects/<name>/<name>.md` (project name as filename, never `_project.md` — a leading `_` would place the file outside OKF conformance scope).
...
- Record provenance in frontmatter on every new/updated page:
  - `generated:` — `by:` is the acting agent's `<producer>/<version>` id (e.g. `codex/0.21.0`); fall back to `okf-wiki/<version>` when the producer/version is unknown. `at:` is the write timestamp.
  - `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:codex-history/<session-id>` (one entry per rollout or index file the knowledge was distilled from).
...
**`_cache/hot.md`** — Read `$OKF_BUNDLE_PATH/_cache/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Ingested 12 Codex sessions; surfaced recurring patterns in CLI tooling and shell scripting." Keep the last 3 operations. Update `updated` timestamp.
...
Derived from Ar9av/obsidian-wiki codex-history-ingest skill (MIT).

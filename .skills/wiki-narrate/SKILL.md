...
description: >
  ...
  lecture requests that must stay within the evidence compiled in an OKF bundle.
---
...
1. Resolve configuration with the Config Resolution Protocol, including an inline
   `@name` bundle override, then read the target bundle's `AGENTS.md` when it exists.
2. Read `_cache/hot.md` and `index.md` first. Select candidates by frontmatter and summary
...
3. When configured, use QMD before `rg`; if QMD is absent, unconfigured, or fails,
   ...
   evidence: establish each claim from the allowed bundle page itself.
...
5. Exclude `_readouts/`, `_raw/`, `_archives/`, `_meta/`, `index.md`, `log.md`,
   `_cache/hot.md`, and `_insights.md` from candidates.
6. Read matching sections before full pages, and read full pages only when a factual
   claim cannot otherwise be established. Preserve relevant status and freshness
   annotations; do not upgrade a page's trust.

## Claim Ledger and Citation Audit

Draft a ledger before prose. Each item contains a claim, supporting file-relative
page links, and one status: supported fact, inferred connection, or ambiguous conflict.
...
`_readouts/<slug>.md` with OKF v0.2 frontmatter: `type: Readout`, `title`, `topic`,
`voice`, `sources` (with mandatory `resource` entries), and `generated` (`{by, at}`).
Use a deterministic, filesystem-safe `<slug>` derived from the topic. Save the same
completed Markdown readout that was returned in conversation.

A readout is an operational artifact: `_readouts/` is out of OKF conformance scope.
Readouts are excluded from retrieval and must not update `index.md` or
`.manifest.json`. Do not create `_readouts/` or a readout file without a successful
`--save` result.
...
- Without `--save`, append the event with `saved=false` after returning the readout;
  do not create a readout or change `_cache/hot.md`.
- After a successful `--save` write, append the event with `saved=true`, then refresh
  `_cache/hot.md` with the topic, voice, cited pages, inference count, evidence gaps, and
  saved readout path. `_cache/hot.md` changes only after a successful save.
...
- **No matching pages:** explain that the bundle lacks material for the topic. Create
  no readout file even if `--save` was requested, do not change `_cache/hot.md`, and record a
...
- **Write failure:** never leave a partial readout presented as saved, never update
  `_cache/hot.md`, and never update `index.md` or `.manifest.json`.

---

> Derived from Ar9av/obsidian-wiki wiki-narrate skill (MIT).

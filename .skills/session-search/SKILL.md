...
description: >
  ...
  never writes to the bundle. Requires a graph built by the session-brain skill.
---
...
```bash
okf-wiki sessions-query "<topic>" --json
```
...
Useful filters: `--project NAME`, `--cluster N`, `--since DATE`, `--top N`. Pass `--out` when the
sidecar lives outside the default `~/.config/okf-wiki/session-graph/`.
...
a structured view of the session node:

```bash
# transcript path is in the query result as `transcript`
okf-wiki sessions-show <session-id> --pretty
```
...
- `wiki-query` — search compiled bundle knowledge; this skill searches raw session history instead
- `wiki-agent` — pull a topic out of *another* agent's history and distil it into the bundle

---

Derived from Ar9av/obsidian-wiki session-search skill (MIT).

...
name: wiki-status
description: >
  Show the current state of the bundle — what's been ingested, what's pending, and the delta between sources
  and bundle content. Use this skill when the user asks "what's the status", "how much is ingested",
  ...
  the bundle itself to surface top hubs, cross-domain bridges, and orphan-adjacent pages.
  Also includes an equilibrium mode triggered by "is my bundle at equilibrium", "wiki equilibrium",
  "is maintenance done", "is the bundle converged", or "are my skills fighting" — runs every maintenance
  skill's audit-only pass and reports whether any of them still has a pending change.
---

# Wiki Status — Audit & Delta

You are computing the current state of the bundle: what's been ingested, what's new since last ingest, and what the delta looks like. This helps the user decide whether to append (ingest the delta) or rebuild (archive and reprocess everything).
...
1. **Resolve config** — follow the Config Resolution Protocol in `okf-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → global config → prompt setup). This gives `OKF_BUNDLE_PATH`, `OKF_SOURCES_DIR`, `CLAUDE_HISTORY_PATH`, and `CODEX_HISTORY_PATH`. The global config dir is XDG-style (`~/.config/okf-wiki`; legacy `~/.obsidian-wiki` installs keep working). Then read `$OKF_BUNDLE_PATH/AGENTS.md` if it exists — owner conventions override framework defaults.
2. Read `.manifest.json` at the bundle root — this is the ingest tracking ledger

Page fields this skill reads (`type`, `status`, `updated`, `tier`, `stale_after`, `visibility/*` tags) follow the canonical frontmatter defined in `okf-wiki/SKILL.md` → *Canonical Frontmatter (Schema)*. This skill consumes them and never redefines them.

## The Manifest

The manifest lives at `$OKF_BUNDLE_PATH/.manifest.json`. It tracks every source file that has been ingested. If it doesn't exist, this is a fresh bundle with nothing ingested.

> **Source keys are canonical absolute paths** (`~` and env vars expanded). Never mix `~`-relative and absolute keys — the same file would be tracked twice and re-ingested. See `okf-wiki/SKILL.md` → *Reserved Files* → `.manifest.json`. Repair a mixed manifest with `python3 scripts/manifest.py normalize <bundle>`.

`.manifest.json` and its sibling `.manifest.lock` are dot-files at the bundle root — operational artifacts outside OKF conformance scope (see *Out-of-Scope Convention* in `okf-wiki/SKILL.md`). `.manifest.lock` is the advisory lock for concurrent writers: `okf-wiki cache-update` takes it and writes atomically (see `wiki-ingest`); never hand-edit the manifest while another agent may be writing.
...
- `content_hash` is the SHA-256 of the file contents at ingest time (`sha256:<64-char-hex>`) — the primary skip signal for delta classification.
- `last_commit_synced` (top level) is maintained by `wiki-update` — the last git commit it synced into the bundle. Read it, leave it untouched.
- `source_type` is advisory (`document`, `image`, `data`, `claude_conversation`, `codex_rollout`, …); `project` ties the entry to a `projects/` scope.

## Step 1: Scan Current Sources

Build an inventory of everything available to ingest right now:

### Documents (from `OKF_SOURCES_DIR`)
```
Glob each directory in OKF_SOURCES_DIR for all text files
...
| **Deleted** | In manifest, but file no longer exists on disk | Note it — bundle pages may be stale |

When a manifest entry has no `content_hash` (older entry), fall back to mtime comparison only.

**Accelerator:** `python3 scripts/manifest.py delta <bundle> --scan '<glob>'` lists NEW/MODIFIED files against the manifest for one scan root (honors `$WIKI_SKIP_PROJECTS` and `--skip`). Run it per source root and merge the results with the classification above.
...
**Visibility tally (before rendering the report):** Grep frontmatter across all bundle `.md` pages for `visibility/internal` and `visibility/pii` tag values. Count:
...
Include this in the Overview section as `Page visibility: N public · M internal · K pii`. Skip the line if all pages are untagged (fully public bundle).

Present a clear summary — **staged writes come first** when staging is enabled and anything is pending (highest intent signal: the work exists, it just needs review):

```markdown
# Wiki Status

## Overview
- **Staged writes pending:** 4 pages · 2 patches (oldest: 3 days ago)  ← only when WIKI_STAGED_WRITES=true; omit the line when staging is off or nothing is pending
- **Total bundle pages:** 87 across 7 categories
...
### New projects (not yet in bundle): 2
...
| **Full bundle (all)** | **142** | **144,200** |

Index-only pass (frontmatter + descriptions): ~8,900 tokens
Typical query (index + 5 full pages):      ~14,200 tokens

⚠️  Full bundle exceeds 100K tokens. Consider:
...
*Token estimates use the 4 chars/token heuristic — no tokenizer involved.*
```
...
3. **Index-only estimate** — Estimate the cost of an index-only pass: sum `len(title) + len(description) + len(tags)` for each page frontmatter (~100 chars each on average), divided by 4.

4. **Typical query estimate** — Index-only estimate + average full-read cost of 5 pages (`total_chars / total_pages * 5 / 4`).

5. **Threshold check** — Read `WIKI_TOKEN_WARN_THRESHOLD` from config (default: `100000`). If `0`, skip the warning. If full-bundle token estimate exceeds the threshold, emit a `⚠️` warning with the three remediation suggestions shown in the template above.
...
0. **Staged writes pending** (only when `WIKI_STAGED_WRITES=true`) — Glob `$OKF_BUNDLE_PATH/_staging/**/*.md` and `**/*.patch.md`. Count new pages and patches separately. Report the oldest file's age (mtime). This is always listed first if any staged files exist — it has the highest intent signal (the LLM already did the work; the human just needs to review).

1. **`_raw/` files** — list every file at the top level of `$OKF_BUNDLE_PATH/_raw/` that isn't a `.gitkeep`, excluding the `_archived/` subdirectory (already-promoted drafts kept for provenance, not pending work). Count and name them.

2. **Stale core pages** — scan all bundle `.md` files. A page is "stale" when its `updated` frontmatter field is ≥90 days before today's date AND it has ≥5 incoming links (i.e., it's "core" — other pages depend on it). List them by name + last-updated date.

3. **Orphan pages** — pages with zero incoming links. To compute: glob all `.md` pages, extract every markdown link target (`.md` links in bodies and `relationships:` targets), count references to each page, collect pages with `incoming == 0`. Show up to 5 names; report total count.

4. **Synthesis opportunities** — check `_cache/hot.md` for any recent `/wiki-synthesize` run summary. If the last synthesis run reported N opportunities, surface that count. If no synthesis has been run recently (not in `_cache/hot.md` or `log.md` within last 14 days), flag it as "synthesis scan overdue".
...
| 3 | Orphan pages | Any pages with zero incoming links |
...
2. 🔄  Refresh 2 stale core pages (not updated in 90+ days)
   → [System Architecture](./concepts/system-architecture.md) (last updated 2026-02-10), [API Design](./concepts/api-design.md) (2026-01-15)
   run: open these pages and re-run /wiki-update

3. 🔗  Link 7 orphan pages  →  run: /cross-linker
   Disconnected: [Redis Caching](./concepts/redis-caching.md), [JWT Tokens](./concepts/jwt-tokens.md), +5 more

4. 🧩  2 synthesis opportunities identified  →  run: /wiki-synthesize
   [Redis Caching](./concepts/redis-caching.md) × [Session Management](./concepts/session-management.md) (co-occur in 8 pages)
...
✅  Bundle is healthy — nothing urgent.
...
Triggered when the user asks something like "wiki insights", "what's central in my wiki", "show me the hubs", "cross-domain bridges", "what pages are most important", or "wiki structure". This mode is *additive* — it doesn't replace the delta report, it analyzes the *shape* of the bundle itself.
...
**First, run the graph analyser.** This replaces manual link parsing — one command produces all the raw data you need:

```bash
okf-wiki graph-analyse "$OKF_BUNDLE_PATH" --pretty --snapshot \
  --diff-against "$OKF_BUNDLE_PATH/_insights.md"   # omit if no previous _insights.md
```

The analyser runs the same algorithm family graphify applies to code graphs, in pure Python: degree ranking, community detection (Leiden if `okf-wiki[graph]` is installed, else label propagation), community cohesion, Brandes betweenness centrality, cross-community surprise scoring, and snapshot diffing. Bundle bookkeeping files (`index`, `log`, `_cache/hot`, `_insights`, `_meta/`, `_readouts/`) are excluded so they don't dominate every ranking.
...
**Fallback** (if `okf-wiki` is not installed): glob all `.md` pages, extract every markdown link target, and build `incoming`, `outgoing`, and `tags` maps manually, then approximate the sections below by hand.
...
4. **Surprising connections.** Cross-community links that are non-obvious.
   ...
   - Re-rank the candidates with content signals the analyser can't see:
     ...
     - **+2** if the types are in different knowledge layers (e.g., `Concept` ↔ `Entity` more surprising than `Concept` ↔ `Concept`)
...
7. **Graph delta since last run.** Read straight from the `diff` field (produced by `--diff-against` on the previous `_insights.md`, which the analyser reads from its `<!-- GRAPH_SNAPSHOT: ... -->` comment):
   - `summary` → "+N pages, +M links"; list `added_pages` / `removed_pages`
...
8. **Tier assignment suggestions.** After computing hubs and bridges, recommend `tier:` changes. Never write `tier:` to pages — only surface suggestions so the human can decide.
   ...
   - Show up to 10 suggestions (promotions first, then demotions), formatted as:
     ```
     Tier Suggestions:
     ↑ core    [attention-mechanism](./concepts/attention-mechanism.md) — 14 incoming links, currently tier=supporting
     ↑ core    [andrej-karpathy](./entities/andrej-karpathy.md)     — bridge (3 cluster pairs), currently unset
     ↓ peripheral [old-concept](./concepts/old-concept.md)       — 0 incoming, 120 days stale
     ```
...
9. **Suggested questions.** Questions this bundle structure is uniquely positioned to answer — or that reveal gaps:
...
Write the result to `_insights.md` at the bundle root. Overwrite freely — it's regenerable. At the very end, embed the analyser's `snapshot` field verbatim as an HTML comment so the next run can diff against it with `--diff-against`.
...
|---|---|---|---|
| [transformer-architecture](./concepts/transformer-architecture.md) | 23 | 8 | connector hub |
| [andrej-karpathy](./entities/andrej-karpathy.md) | 17 | 0 | sink hub — cross-linker candidate |

## Bridge Pages (top 5)
| Page | Bridges | Cross-cluster pairs |
|---|---|---|
| [exponential-growth](./concepts/exponential-growth.md) | #ml ↔ #economics | 4 pairs |
...
- [scaling-laws](./concepts/scaling-laws.md) → [gordon-moore](./entities/gordon-moore.md) — score 5
  - Reason: cross-layer (Concept ↔ Entity), marked ^[inferred]
- ...

## Orphan-Adjacent (dead-ends near hubs)
- [foo](./concepts/foo.md) — linked from 3 hubs, 0 outbound links
...
- +3 new pages, +11 new links
- Newly connected: [bar](./concepts/bar.md), [baz](./entities/baz.md)
- Lost incoming links: [old-paper](./references/old-paper.md) (target may have been renamed)

## Tier Suggestions
↑ core    [attention-mechanism](./concepts/attention-mechanism.md) — 14 incoming links, currently tier=supporting
↑ core    [andrej-karpathy](./entities/andrej-karpathy.md)     — top bridge (4 cluster pairs), currently unset
↓ peripheral [old-concept](./concepts/old-concept.md)       — 0 incoming, 132 days stale
...
<!-- GRAPH_SNAPSHOT: <the analyser's snapshot JSON, embedded verbatim — nodes, edges, communities> -->
```
...
- Bundles with fewer than 20 pages — not enough graph structure. Tell the user and skip.
- After a fresh `wiki-rebuild` — wait until at least one ingest has happened.

## Equilibrium Mode

Triggered when the user asks "is my bundle at equilibrium", "wiki equilibrium", "is the bundle converged", "is maintenance done", "are my skills fighting", or "what's still pending across all the maintenance skills".

The maintenance skills are **players in a game over one shared bundle**. Each has a move set (lint fixes, dedup merges, new links, tag normalizations) and each judges the bundle by its own objective. The bundle is at **equilibrium** when no player has a profitable deviation — every audit pass proposes zero changes. That is the real "maintenance is done" signal, and it is stronger than any single skill reporting clean, because skills can undo each other.
...
|---|---|---|
| `wiki-lint` | `okf-wiki lint "$OKF_BUNDLE_PATH" --json` | each finding in `findings` (sum the `stats.findings` counts) |
...
```
# Bundle Equilibrium — <TIMESTAMP>

equilibrium: no

| Player | Pending moves | Top proposed move |
|---|---|---|
| wiki-lint | 4 | fix broken link `./concepts/old-name.md` referenced in `synthesis/foo.md` |
...
A player whose count keeps returning after being driven to zero means two players are **fighting** — the classic case is `tag-taxonomy` normalizing a tag that an ingest skill's defaults keep re-adding, so the bundle cycles forever and never converges.
...
Pending moves are not evenly distributed across sources. Attribute them back: for each page appearing in a player's findings, look up which manifest source produced it (`.manifest.json` → `pages_created` / `pages_updated`).
...
Derived from Ar9av/obsidian-wiki wiki-status skill (MIT).

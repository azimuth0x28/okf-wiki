...
name: wiki-query
description: >
  Answer questions by searching the compiled OKF bundle. Use this skill when the user asks a question
  about their knowledge base, wants to find information across their bundle, asks "what do I know about X",
  "find everything related to Y", or wants synthesized answers with citations from their bundle pages.
  Also use when the user wants to explore connections between topics in their bundle, or asks a multi-hop
  ...
  "fast lookup" — returns answers from page descriptions and frontmatter without reading page bodies.
  Citations carry OKF trust context (status, verified, stale_after) and claim markers; visibility
  filtering ("public only") is opt-in. Accepts inline named-bundle routing like "wiki-query @work what
  do I know about X" via the shared Config Resolution Protocol.
---

# Wiki Query — Knowledge Retrieval

You are answering questions against a compiled OKF bundle, not raw source documents. The bundle contains pre-synthesized, cross-referenced knowledge.

## This skill is READ-ONLY

`wiki-query` answers questions. It MUST NOT create or modify any bundle content. The ONLY write it may perform is the single Step 6 append to `log.md`.
...
- modify `index.md`, `_cache/hot.md`, `_insights.md`, or `.manifest.json`
...
1. **Resolve config** — follow the Config Resolution Protocol in `okf-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → global config → prompt setup). For cross-project queries without `@name`, prefer the global config when present, even if it is a symlink to the bundle `.env`. This gives `OKF_BUNDLE_PATH` and any QMD variables. Works from any project directory. Then read `$OKF_BUNDLE_PATH/AGENTS.md` if it exists — owner conventions override framework defaults.
...
3. If `$OKF_BUNDLE_PATH/_cache/hot.md` exists, read it first — it gives you instant context on recent activity. If the user's question is about something ingested recently, `_cache/hot.md` may answer it before you even open `index.md`.
4. Read `$OKF_BUNDLE_PATH/index.md` to understand the bundle's scope and structure
...
Pages with no `visibility/` tag, or tagged `visibility/public`, are always included. `visibility/` tags are **system tags** — they never count toward the 5-tag limit and are listed separately from domain/type tags in `_meta/taxonomy.md`.

In filtered mode, note the filter in the Step 6 log entry: `mode=filtered`.

## Retrieval Protocol

**Follow the Retrieval Primitives table in `okf-wiki/SKILL.md`.** Reading is the dominant cost of this skill — use the cheapest primitive that answers the question and escalate only when it can't. Never jump straight to full-page reads.
...
```bash
okf-wiki graph-query "$OKF_BUNDLE_PATH" "<question>" --pretty
```

Output fields:

- **`answer_type`**: `direct` | `path` | `list` | `gap` | `impact` | `bridges` | `hubs` | `clusters` | `surprising` — shapes what to do next. The last five are **structural intents**: the question is about the shape of the bundle, and the answer comes back fully computed in `graph` (see below) with no page reads at all.
- **`graph`**: present only for structural intents — the computed answer. `null` otherwise.
- **`candidates`**: top-ranked pages by title/tag/description match + degree, with scores and descriptions
- **`should_read`**: the pages most worth opening — start here instead of speculatively reading many files
- **`path`**: for multi-hop queries, the shortest link path between the two concepts
- **`god_nodes_relevant`**: hub pages related to your query terms — always useful context
- **`index_only`**: if `true`, the top candidate's description already answers the question — skip page reads
...
| "which pages bridge my clusters" / "what would fragment my bundle" | `bridges` | pages by betweenness, with `label` and `connects_labels` |
| "what's central" / "top hubs" / "my main topics" | `hubs` | pages by degree, with in/out split |
| "what clusters do I have" / "how is my bundle organised" | `clusters` | each cluster's `label`, `size`, `cohesion`, `fragmented` |
...
2. If `index_only: true` → answer directly from `candidates[0].description`. Skip Steps 1–4, go to Step 5.
...
> The graph used here excludes bundle bookkeeping files (`index.md`, `log.md`, `_cache/hot.md`, `_insights.md`). They link to nearly every page, so including them made any two pages look ~2 hops apart and produced meaningless `A → index → B` paths.

**Fallback** (if `okf-wiki` is not installed): proceed with Step 1 as normal using grep and index.md.
...
- Use `Grep` to scan page **frontmatter only** for title, tag, alias, and description matches. A pattern like `^(title|tags|aliases|description):` scoped to bundle `.md` files is far cheaper than content grep.
- Collect the top 5–10 candidate page paths ranked by:
  1. Exact title or alias match
  2. Tag match
  3. `description` field contains the query term
...
If you're in **index-only mode**, stop here. Answer from `description:` fields, titles, and `index.md` descriptions only. Label the answer clearly: **"(index-only answer — page bodies not read; facts below are from page descriptions and may miss nuance)"**. Then skip to Step 5.
...
```
mcp__qmd__query:
  collection: <QMD_WIKI_COLLECTION>   # e.g. "knowledge-base-bundle"
...
If `QMD_PAPERS_COLLECTION` is set and the user is asking about a topic likely covered by ingested papers (research, theory, background), run a parallel search against the papers collection. Cite raw sources separately from compiled bundle pages in your answer.
...
- Follow at most one hop of markdown links from those pages if the answer requires cross-references.
...
- If you're still short, **then** fall back to a broad content grep across the bundle. Tell the user you escalated — this is the expensive path and they should know.
...
1. **Build the typed-edge adjacency (cheap).** Grep every page's `relationships:` block in one pass — `Grep -A 20 "^relationships:" <bundle>/**/*.md` (frontmatter only). Each entry yields a directed, typed edge `source —type→ target`. Add the reverse direction as a traversable edge too (mark it `(reverse)`), since "connected to" is symmetric even though the typed assertion is directional. Plain body markdown links (`[display](./path.md)`) count as untyped `related_to` edges only if you need them to complete a path — prefer typed edges first.
...
3. **Bounded BFS.** If `okf-wiki` is installed, let the CLI do the walk over the link graph first — it is exact and instant:

   ```bash
   okf-wiki graph-analyse "$OKF_BUNDLE_PATH" --path "<X>" "<Y>"                  # two-endpoint: shortest chain + hop count
   okf-wiki graph-analyse "$OKF_BUNDLE_PATH" --around "<X>" --depth 3 [--direction out|in]  # one-endpoint: reachable pages by hop
   ```

   `--path` follows links in either direction unless `--direction out`; `--around --direction in` answers "what depends on X" (its blast radius). Then decorate the returned chain with the typed edges from step 1 (the CLI sees markdown links, not relationship types). Otherwise, or to find alternate paths, walk manually:
   ...
   - **Frontier cap:** stop expanding a node once the visited set exceeds ~60 pages — report partial results rather than fanning out across the whole bundle.
...
4. **Report the path(s) with edge types.** Show the chain, not just the endpoints — the typed edges *are* the answer:
   ...
   ```
   [transformers](./concepts/transformers.md) —uses→ [attention](./concepts/attention.md) —derived_from→ [rnn-seq2seq](./concepts/rnn-seq2seq.md) —contradicts (reverse)→ [lstm](./concepts/lstm.md)
   ```
...
Compose your answer from bundle content:
- Cite specific bundle pages using file-relative markdown links (`[topic](./concepts/topic.md)`)
- Note which step the answer came from ("found in description" vs "grepped section" vs "full page read") — helps the user understand confidence
- If the bundle has contradictions, present both sides
- If the bundle doesn't cover something, say so explicitly
- Suggest which sources might fill the gap

**Page trust annotations (OKF trust context).** For every page cited in your answer, read its `status`, `verified`, and `stale_after` frontmatter and annotate risky pages inline so the user knows which citations to verify. A page is stale when `now >= stale_after` (`stale_after = updated + 90d`, computed at write time — see *Lifecycle* in `okf-wiki/SKILL.md`):

| Condition | Annotation |
|---|---|
| `status: deprecated` (+ `superseded_by`) | `(DEPRECATED: superseded by [target](./path/to/target.md))` — use the successor instead |
| extension `lifecycle: disputed` | `(DISPUTED, marked <lifecycle_changed>: <lifecycle_reason or "reason unspecified">)` — overrides every state except deprecated in display |
| `status: draft` | `(draft — unreviewed)` |
| stale + `verified` entry by a `human:<id>` actor | `(human-reviewed by <id> but stale: stale_after <date>)` — reader should re-verify before relying |
| stale (any other status) | `(stale: since <stale_after>)` |

When a page carries `verified:` entries, cite the strongest one: an entry `by: human:<id>` means **human-reviewed**; entries only by `process:<id>` or agent actors mean machine-confirmed at best (see *Trust tiers* in `okf-wiki/SKILL.md`). Absent `status` defaults to `stable` per OKF v0.2; absent `stale_after` means the staleness rule can't fire. Never fabricate a `lifecycle_reason`; if the field is absent, omit the reason from the annotation.

Examples in a synthesized answer:
```
[concept-page](./concepts/concept-page.md) (stale: since 2026-01-15) — Original claim was X.
[verified-page](./references/verified-page.md) (human-reviewed by human:ana but stale: stale_after 2025-09-10) — Reader should re-verify before relying.
[disputed-page](./concepts/disputed-page.md) (DISPUTED, marked 2026-04-30: contradicted by [new-source](./references/new-source.md)) — Earlier said Y, now uncertain.
[old-page](./concepts/old-page.md) (DEPRECATED: superseded by [new-page](./concepts/new-page.md)) — Use the successor.
[new-page](./concepts/new-page.md) (draft — unreviewed) — Promising, but a human has yet to review it.
```

**Surface claim markers.** When you quote or paraphrase a page's claims, carry its truth-state markers into your answer verbatim — `^[inferred]` for agent-synthesized claims and `^[ambiguous]` for contested or unclear ones (the truth-state axis in `okf-wiki/SKILL.md`). A cited claim marked `^[ambiguous]` should present both sides where the page does; an answer whose key chain rests on `^[inferred]` claims should say so, so the reader knows which parts are distilled fact and which are synthesis.
...
1. Read `$OKF_BUNDLE_PATH/.manifest.json` and look up `.projects.<name>.source_cwd` — this is the authoritative path.
...
> **Based on the bundle:**
>
> [Your synthesized answer with file-relative markdown links to source pages]
>
> **Pages consulted:** [page-a](./concepts/page-a.md), [page-b](./entities/page-b.md), [page-c](./references/page-c.md)
>
> **Gaps:** [What the bundle doesn't cover that might be relevant]
>
> **Retrieval:** N candidates seen, M read, D dropped (untouched matches, if any: [page-d](./concepts/page-d.md), [page-e](./concepts/page-e.md))
>
...
Derived from Ar9av/obsidian-wiki wiki-query skill (MIT).

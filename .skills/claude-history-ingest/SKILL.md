...
description: >
  Ingest Claude Code conversation history into the OKF knowledge bundle. Use this skill when the user wants to mine
  their past Claude conversations for knowledge, import their ~/.claude folder, extract insights from
  previous coding sessions, or says things like "process my Claude history", "add my conversations to the bundle",
...
You are extracting knowledge from the user's past Claude Code conversations and distilling it into the OKF knowledge bundle. Conversations are rich but messy — your job is to find the signal and compile it.
...
1. **Resolve config** — follow the Config Resolution Protocol in `okf-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → global config → prompt setup). This gives `OKF_BUNDLE_PATH` and `CLAUDE_HISTORY_PATH` (defaults to `~/.claude`). Legacy `~/.obsidian-wiki` config is still honored; new installs use `~/.config/okf-wiki`. Then read `$OKF_BUNDLE_PATH/AGENTS.md` if it exists — owner conventions override framework defaults.
2. Read `.manifest.json` at the bundle root to check what's already been ingested
3. Read `index.md` at the bundle root to know what the bundle already contains
...
> **Canonical paths when comparing.** The manifest keys are absolute paths with `~` expanded (see `okf-wiki/SKILL.md` → `.manifest.json`). Before deciding a file is "new", expand its path the same way — otherwise a file already tracked as `~/.claude/...` looks new when you scanned it as `/Users/me/.claude/...` (or vice-versa) and gets re-ingested. The `scripts/manifest.py` helper does this for you:
>
> ```bash
> # New/modified sources, honoring WIKI_SKIP_PROJECTS + --skip, paths already canonical:
> python3 "$OKF_WIKI_REPO/scripts/manifest.py" delta "$OKF_BUNDLE_PATH" \
>   --scan "$CLAUDE_HISTORY_PATH/projects/*/memory/*.md"
> # One-time repair if the manifest already mixes ~ and absolute keys:
> python3 "$OKF_WIKI_REPO/scripts/manifest.py" normalize "$OKF_BUNDLE_PATH" --dry-run
> ```
>
...
python3 "$OKF_WIKI_REPO/scripts/extract-jsonl.py" --skip tsg,autom8

# Incremental — only sessions modified in the last day
python3 "$OKF_WIKI_REPO/scripts/extract-jsonl.py" \
...
1. **Memory files** (`~/.claude/projects/*/memory/*.md`) — Pre-distilled, already bundle-friendly. Gold.
...
Don't create one bundle page per conversation. Instead:
...
## Step 5: Distill into Bundle Pages

Each Claude project maps to a project directory in the bundle. The project directory name from `~/.claude/projects/` encodes the original path — decode it to get a clean project name:
...
For each project with content, create or update the project overview page at `projects/<name>/<name>.md` — **named after the project, never `_project.md`**: a leading `_` would place the file outside OKF conformance scope, and naming the file after the project yields a distinct, readable page ID for every project.
...
**Record provenance in frontmatter** on every new/updated page:

- `generated:` — `by:` is the acting agent's `<producer>/<version>` id (e.g. `claude/2.5.0`); fall back to `okf-wiki/<version>` when the producer/version is unknown. `at:` is the write timestamp.
- `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:claude-history/<session-id>` (one entry per conversation, memory file, or audit log the knowledge was distilled from).

**Write a `description:` frontmatter field** on every new/updated page — 1–2 sentences, ≤200 chars, answering "what is this page about?" for a reader who hasn't opened it. `wiki-query`'s cheap retrieval path reads this field to avoid opening page bodies.

**Add confidence and status fields** to every new page's frontmatter (canonical OKF v0.2 — see `okf-wiki/SKILL.md` §Schema):
```yaml
base_confidence: 0.42
status: draft
lifecycle_changed: <ISO date today>
stale_after: <updated + 90d>
```
On update, leave `status` and `lifecycle_changed` unchanged — only a human editor transitions lifecycle state (`draft → stable` is human-only).

**Mark provenance** per the convention in `okf-wiki/SKILL.md` (Provenance Markers section):
...
```json
{
  "project-name": {
    ...
    "bundle_path": "projects/project-name",
...
- The user's conversations may reference other people — be thoughtful about what goes in the bundle
...
Derived from Ar9av/obsidian-wiki claude-history-ingest skill (MIT).

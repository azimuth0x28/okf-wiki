...
```bash
okf-wiki code-understand --project <dir> [--backend auto|builtin|codegraph] \
...
4. **It's a cache, not a deliverable.** Never write the raw JSON or `.codegraph/` into the
   bundle, a PR description, or committed docs — it's a disposable sidecar in the project
...
Check `okf-wiki doctor --project <dir>` for the `code-understanding.*` capability lines
...
---
Derived from Ar9av/obsidian-wiki code-understand skill (MIT).

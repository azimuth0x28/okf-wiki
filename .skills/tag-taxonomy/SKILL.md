...
name: tag-taxonomy
description: >
  Enforce consistent tagging across the wiki using a controlled vocabulary.
...
> **Format not platform.** This skill operates on an OKF v0.2 bundle. Obsidian is an optional viewer; the bundle's markdown is the source of truth.
...
2. Read `$OKF_BUNDLE_PATH/_meta/taxonomy.md` — this is the canonical tag list
3. Read `index.md` to understand the wiki's scope

## The Taxonomy File

The canonical tag vocabulary lives at `$OKF_BUNDLE_PATH/_meta/taxonomy.md`. It defines:
...
**`_meta/taxonomy.md` is an operational artifact** — it lives outside OKF v0.2 conformance scope. It controls the tag vocabulary for this bundle but is not part of the OKF schema definition. See `.skills/okf-wiki/SKILL.md` §Schema for the canonical frontmatter mapping.

**Always read this file before tagging.** It's the source of truth.

## Reserved System Tags

`visibility/` is a **system tag** group with special rules. System tags are orthogonal to the domain/type taxonomy and are managed separately:
...
**Rules for `visibility/` system tags:**
- They are **system tags** — they do **not** count toward the 5-tag USER tag limit
...
- **Pass through untouched** during normalization — system tags are not subject to alias mapping or taxonomy changes
...
```
Glob: $OKF_BUNDLE_PATH/**/*.md (excluding _archives/, _meta/)
...
- **Over-tagged pages** — pages with more than 5 USER tags (system `visibility/*` tags excluded from the count)
...
3. If page has > 5 USER tags, suggest which to drop (keep the most specific/relevant ones; ignore `visibility/*` tags in the count)
...
2. Select up to 5 USER tags that best describe the page (system `visibility/*` tags are separate):
...
**`_cache/hot.md`** — Read `$OKF_BUNDLE_PATH/_cache/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Tag audit: normalized 14 tags across 28 pages; 2 new canonical tags added." Keep the last 3 operations. Update `updated` timestamp.
...
*Derived from Ar9av/obsidian-wiki tag-taxonomy skill (MIT).*

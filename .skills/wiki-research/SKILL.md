...
description: >
  ...
  results into the OKF bundle. Use this skill when the user says "/wiki-research [topic]",
  "research X", "find everything about Y", "do a deep dive on Z", "autonomous research on X",
  or wants comprehensive, web-sourced knowledge on a topic filed directly into their bundle.
---

# Wiki Research — Autonomous Multi-Round Research

You are running an autonomous research loop on a topic, synthesizing what you find, and filing the results into the OKF bundle as permanent knowledge.
...
2. Read `$OKF_BUNDLE_PATH/index.md` to understand what's already in the bundle — don't re-research things the bundle covers well.
3. Read `$OKF_BUNDLE_PATH/_cache/hot.md` if it exists — it surfaces recent context.
4. Check `$OKF_BUNDLE_PATH/references/research-config.md` if it exists — it may define source preferences, domains to skip, or confidence rules for this bundle.
5. Check `$OKF_BUNDLE_PATH/references/research-backends.md` if it exists — it registers optional CLI retrieval backends (social media, video transcripts, paid APIs, etc.). Load any available backends into your working state for this session.

Internal links in generated pages use file-relative format: `[display](./concepts/topic.md)`. See `.skills/okf-wiki/SKILL.md` for the link format specification.

Confirm the research topic with the user if it's ambiguous. Then proceed.

## Research Configuration (optional)

If `references/research-config.md` exists in the bundle, read it and apply any rules it defines:
...
If `references/research-backends.md` exists in the bundle, load it before starting research. It defines zero or more CLI retrieval backends as a YAML list:
...
If the user asks for an example registry, offer this file at `$BUNDLE/references/research-backends.md`:
...
## Filing — Write Bundle Pages

Organize all findings into bundle pages across four output areas. All pages use OKF v0.2 frontmatter — reference the canonical mapping in `.skills/okf-wiki/SKILL.md` for the full schema. Pages are connected via file-relative links.

### 1. references/ — One page per major reference
...
type: Reference
title: <Source title>
description: <1-2 sentences, ≤200 chars>
tags: [<2-4 domain tags>]
generated:
  by: okf-wiki/research
...
Standard OKF v0.2 concept frontmatter + body. Link concepts to each other and to source pages using file-relative links.

### 3. entities/ — Tools, organizations, people

For each significant entity encountered (tools, libraries, companies, key authors):

Standard OKF v0.2 entity frontmatter. Link back to concepts that use the entity and sources where it appears, using file-relative links.
...
description: Synthesis of <N>-round research on <topic>. Covers <core findings in ≤200 chars>.
...
## Key Findings
<Bulleted list of the most important claims, each with a [source page](./references/source-page.md) citation>
...
All internal links use file-relative format: `[display](./path/to/page.md)`. Check `index.md` for existing pages on the same topics — merge into existing pages rather than creating duplicates.
...
**`_cache/hot.md`** — Update **Recent Activity** with the research topic and core finding. Update **Active Threads** if this is ongoing. Update `updated` timestamp.
...
- [ ] All pages cross-linked with file-relative links
- [ ] `index.md`, `log.md`, `_cache/hot.md`, `.manifest.json` updated
...
Derived from Ar9av/obsidian-wiki wiki-research skill (MIT).

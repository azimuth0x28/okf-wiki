...
This is not bulk ingest. You find sessions about a specific topic, extract the relevant blobs, distill them into the bundle, and return a synthesized answer the user can act on immediately.
...
2. Read `$OKF_BUNDLE_PATH/.manifest.json` → know what's already ingested.
3. Read `$OKF_BUNDLE_PATH/_cache/hot.md` if it exists → warm context on recent bundle activity.
...
4. **Already ingested** — if this session was previously ingested and the bundle page already covers the query (check `_cache/hot.md` + `index.md`), flag as "covered" but still show in results
...
## Step 5: Distill Blobs into Bundle Pages

For each extracted blob, determine where it belongs in the bundle:

1. **Check if a bundle page already covers this** — grep `index.md` and page frontmatter for the topic. If yes, update the existing page rather than creating a new one.
2. **Determine category** using standard rules (from `okf-wiki/SKILL.md`):
   - Technique / how-to → `skills/`
   - Abstract concept / pattern → `concepts/`
   - Tool / library / person → `entities/`
   - Cross-cutting insight → `synthesis/`
3. **Write or update the page** with required OKF v0.2 frontmatter:
   ```yaml
   ---
   title: <topic>
   type: skill|concept|entity|synthesis    # TitleCase, one of the four
   tags: [tag1, tag2]
   sources:
     - resource: "scope:<agent>-history/<session-id>"
   generated:
     by: <producer>/<version>              # e.g. okf-wiki/0.2 or claude/2.5.0
     at: <ISO datetime>
   status: draft                           # agents always create as draft
   stale_after: <updated + 90d, ISO>      # recomputed on every write
   base_confidence: <0.0–1.0>
   tier: <0–5>
   lifecycle_changed: <ISO datetime>
   ---
   ```
   Set `sources` with the scope descriptor so `memory-bridge` can find it later.
4. **Add cross-links** to related bundle pages found in `index.md` — use file-relative markdown links: `[display](./relative/path.md)`.

Distillation rules (same as all ingest skills):
- Extract durable knowledge, not operational telemetry
- One bundle page per concept, not one per session
...
After ingesting, immediately synthesize and return an answer from the newly ingested + existing bundle content:
...
**Bundle pages updated/created:**
- [page-name](./category/page-name.md) — <what was added>
- [page-name](./category/page-name.md) — <what was added>
...
Update `_cache/hot.md` with a one-line summary of what was ingested.
...
→ Run `/wiki-claude "X"`, `/wiki-codex "X"`, `/wiki-hermes "X"` in sequence — each ingests its slice, the bundle accumulates the cross-agent picture, then `/memory-bridge diff` shows what each tool uniquely contributed
...
Derived from Ar9av/obsidian-wiki wiki-agent skill (MIT).

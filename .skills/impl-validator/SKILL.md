...
3. **Correctness check** — does the content logically match the stated goal? Look for:
   ...
   - Off-by-one or empty-set edge cases (e.g. page count = 0 when bundle is known non-empty)
4. **Convention check** — does it follow the project's established patterns?
   ...
   - Bundle pages: carry OKF v0.2 frontmatter — minimum `type` (TitleCase) and `generated`; pages built from sources also carry `sources` with a `resource` entry (canonical mapping: `.skills/okf-wiki/SKILL.md` §Schema)
...
Derived from Ar9av/obsidian-wiki impl-validator skill (MIT).

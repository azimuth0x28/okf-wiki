...
description: >
  Ingest Pi coding agent session history into the OKF knowledge bundle. Use this skill when the user wants to mine
  their past Pi sessions for knowledge, import their ~/.pi/agent/sessions folder, extract insights from
  previous coding sessions, or says things like "process my Pi history", "add my Pi sessions to the bundle",
...
You are extracting knowledge from the user's Pi coding agent sessions and distilling it into the OKF knowledge bundle. Pi sessions are stored as structured JSONL with a tree layout — your job is to follow the active branch, extract durable knowledge, and compile it.
...
1. **Resolve config** — follow the Config Resolution Protocol in `okf-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → global config → prompt setup). This gives `OKF_BUNDLE_PATH` and `PI_HISTORY_PATH` (defaults to `~/.pi/agent/sessions`). Legacy `~/.obsidian-wiki` config is still honored; new installs use `~/.config/okf-wiki`. Then read `$OKF_BUNDLE_PATH/AGENTS.md` if it exists — owner conventions override framework defaults.
2. Read `.manifest.json` at the bundle root to check what has already been ingested
3. Read `index.md` at the bundle root to understand what the bundle already contains
...
As you parse, build a private evidence ledger before writing any bundle page. Each durable fact or decision you may write must carry at least one source reference:
...
Do not create one bundle page per session.
...
Route extracted knowledge using existing bundle conventions:
...
For each impacted project, create/update `projects/<name>/<name>.md` (never `_project.md` — a leading `_` would place the file outside OKF conformance scope).
...
- Record provenance in frontmatter on every new/updated page:
  ...
  - `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:pi-history/<session-id>` (one entry per session file the knowledge was distilled from).
- Add a `description:` frontmatter field on each new/updated page (1–2 sentences, ≤ 200 chars)
- Add confidence and status fields to every new page (canonical OKF v0.2 — see `okf-wiki/SKILL.md` §Schema):
  ```yaml
...
- Add provenance markers using the convention in `okf-wiki/SKILL.md`:
...
**Mark provenance** per the convention in `okf-wiki/SKILL.md`:
...
**`_cache/hot.md`** — Read `$OKF_BUNDLE_PATH/_cache/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Ingested 12 Pi sessions across 3 projects; surfaced patterns in CLI tooling and API design." Keep the last 3 operations. Update `updated` timestamp.
...
Derived from Ar9av/obsidian-wiki pi-history-ingest skill (MIT).

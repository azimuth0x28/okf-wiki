...
name: wiki-rebuild
description: >
  Archive existing bundle knowledge and rebuild from scratch, or restore from a previous archive.
  Use this skill when the user wants to start fresh, rebuild the bundle from all sources, archive current
  knowledge before a major change, or restore an older version. Triggers on "rebuild the bundle",
  "start over", "archive and rebuild", "restore from archive", "nuke and repave", "clean rebuild".
  Also use when the bundle has drifted too far from sources and incremental fixes won't cut it.
---

# Wiki Rebuild — Archive, Rebuild, Restore

You are performing a destructive operation on the bundle. Always archive first, always confirm with the user before proceeding.

## Before You Start

1. **Resolve config** — follow the Config Resolution Protocol in `okf-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → global config → prompt setup). This gives `OKF_BUNDLE_PATH` and optional QMD settings such as `QMD_WIKI_COLLECTION`
2. Read `.manifest.json` to understand current state
3. **Confirm the user's intent.** This skill supports three modes:
   - **Archive only** — snapshot current bundle, no rebuild
   - **Archive + Rebuild** — snapshot, then reprocess all sources from scratch
   - **Restore** — bring back a previous archive

## The Archive System

Archives live at `$OKF_BUNDLE_PATH/_archives/`. Each archive is a timestamped directory containing a full copy of the bundle state at that point. Snapshots are outside OKF conformance scope — no validation runs inside `_archives/`.
...
```json
{
  ...
  "total_sources": 42,
  "total_projects": 6,
  "bundle_path": "/Users/name/MyBrain",
...
4. Append to `log.md`:
   ...
   ```
5. Optionally refresh QMD if `log.md` is indexed and `QMD_WIKI_COLLECTION` is configured (see "QMD Refresh After Live Bundle Changes").
6. Report: "Archived 87 pages. Current bundle is untouched."
...
### Step 2: Clear live bundle

Remove all content from the category directories (`concepts/`, `entities/`, `skills/`, etc.) and the `projects/` directory. Keep:
- `_archives/` (obviously)
- `_raw/` (source materials and staging)
- `_cache/`, `_readouts/`, `_meta/`, `_staging/` (operational out-of-scope dirs)
- `_insights.md`
- `.env` (if present in bundle)
- `AGENTS.md` (owner conventions)
- `okf-base.yaml` (okflint profile)
...
Tell the user the bundle is cleared and ready for a full re-ingest. They can now run:
...
Refresh QMD after clearing and logging the live bundle (see "QMD Refresh After Live Bundle Changes"), then report that the bundle is ready for selected re-ingest skills.
...
Ask the user which archive they want. Warn them that restoring will overwrite the current live bundle.
...
1. Clear the live bundle (same as Mode 2, Step 2)
2. Copy all content from the chosen archive back into the live bundle directories
...
Refresh QMD after restore (see "QMD Refresh After Live Bundle Changes"), then tell the user what was restored and suggest running `wiki-lint` to check for any issues with the restored state.

## QMD Refresh After Live Bundle Changes

QMD is a search index, not the source of truth. If QMD refresh fails, do not roll back archive, rebuild, or restore work; report the failure and leave the markdown bundle intact.
...
|---|---|---|
| Archive only | Optional | Live bundle content is unchanged except `log.md`; refresh if `log.md` is indexed and QMD is configured. |
| Archive + Rebuild | Required after clearing live bundle | QMD must forget deleted pages or it will return stale search results. Later ingest skills will refresh again as sources are reprocessed. |
| Restore | Required after restore | The live bundle was replaced with archive content, so QMD must match the restored state. |
...
Verify the bundle collection reflects the operation:
...
2. **Always confirm with the user** before clearing the live bundle.
3. **Never delete archives** unless the user explicitly asks. Archives are cheap insurance.
4. **Out-of-scope directories are preserved.** Never touch `_raw/`, `_cache/`, `_readouts/`, `_meta/`, `_staging/`, or dot-files during archive/rebuild/restore — they contain operational artifacts outside OKF conformance scope.
5. If something goes wrong mid-rebuild, the archive is there. Tell the user they can restore.

---

> Derived from Ar9av/obsidian-wiki wiki-rebuild skill (MIT).

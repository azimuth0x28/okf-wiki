...
description: >
  Ingest GitHub Copilot CLI session history into an OKF knowledge bundle as distilled knowledge pages. Use this skill
  when the user wants to capture their Copilot CLI sessions into a personal knowledge base — extracting architecture
  decisions, debug notes, and patterns into searchable bundle pages. Triggers on phrases like "ingest my
  copilot sessions into my knowledge base", "add my copilot history to my wiki", "pull my copilot session history into
  the bundle", "capture what I've learned from copilot", "just the new sessions since last time",
...
You are extracting knowledge from the user's past GitHub Copilot CLI conversations and distilling it into the OKF knowledge bundle. Conversations are rich but messy — your job is to find the signal and compile it.
...
**Session refs** link Copilot sessions to git history — useful for connecting bundle knowledge to concrete code changes.

## Step 4: Cluster by Topic

Don't create one bundle page per session. Instead:
...
Each Copilot project maps to a project directory in the bundle. Derive the project name from `cwd` or `repository`:
...
- `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:copilot-history/<session-id>` (one entry per session, checkpoint, or memory artifact the knowledge was distilled from).
...
```
Leave `status` unchanged on update — only a human editor transitions lifecycle state (`draft → stable` is human-only).
...
- `data.reasoningOpaque` / `data.reasoningText` in assistant events is internal reasoning — skip entirely, never copy to the bundle
...
Derived from Ar9av/obsidian-wiki copilot-history-ingest skill (MIT).

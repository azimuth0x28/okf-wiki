...
description: >
  Ingest Hermes agent history into the OKF knowledge bundle. Use this skill when the user wants to mine
  ...
  memories to the bundle", "ingest ~/.hermes", or "what have I worked on in Hermes". Also triggers
...
You are extracting knowledge from the user's Hermes agent history and distilling it into the OKF knowledge bundle. Hermes stores both free-form memories and structured session transcripts — focus on durable knowledge, not operational telemetry.
...
- Note any tags Hermes attached (they often map to bundle categories)
- Merge into the appropriate bundle page rather than creating one memory = one page
...
Do not create one bundle page per memory or session.
...
- Record provenance in frontmatter on every new/updated page:
  ...
  - `sources:` — every entry derived from agent history uses a scope descriptor: `resource: scope:hermes-history/<session-id>` (one entry per memory file or session the knowledge was distilled from).
...
**`_cache/hot.md`** — Read `$OKF_BUNDLE_PATH/_cache/hot.md` (create from the template in `wiki-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Ingested 42 Hermes memories and 7 sessions; dominant themes: reasoning strategies, tool use patterns." Keep the last 3 operations. Update `updated` timestamp.
...
Derived from Ar9av/obsidian-wiki hermes-history-ingest skill (MIT).

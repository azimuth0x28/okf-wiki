...
name: wiki-dedup
description: >
  Scan the OKF bundle for page-level identity collisions — different pages covering the same
  concept under different names (e.g. "RSC" vs "React Server Components") — and merge them.
  Use this skill when the user says "dedup my bundle", "find duplicate pages", "merge duplicates",
  "identity resolution", "consolidate my bundle", "I have duplicate pages", or "my bundle has two pages
...
You are finding and merging OKF bundle pages that cover the same concept under different names. This is a write-heavy, potentially destructive skill — page merges cannot be automatically undone. Work carefully and confirm before acting in merge mode.

**Follow the Retrieval Primitives table in `.skills/okf-wiki/SKILL.md`.** The candidate-detection pass uses only frontmatter and titles (cheap). Only open full page bodies for confirmed candidate pairs.
...
Glob all `.md` files in the bundle (excluding `_archives/`, `_raw/`, `_meta/`, `_cache/`, `_staging/`, `index.md`, `log.md`, `_cache/hot.md`, `_insights.md`, and any file that contains `redirects_to:` in its frontmatter — those are already merged redirect stubs). Skip dot-files and dot-directories.

For each remaining page, extract from frontmatter:
- `node_id` — relative path from bundle root, without `.md`
- `title` — frontmatter `title` field
- `aliases` — frontmatter `aliases` list (may be absent)
- `tags` — frontmatter `tags` list
- `type` — frontmatter `type` field (TitleCase), derived from directory prefix

Build a lookup table: `node_id → {title, aliases, tags, type, summary}`.
...
If the bundle has fewer than 10 pages, skip the pair loop and report "bundle too small to have meaningful duplicates". If the bundle has more than 500 pages, process candidates in batches of 50 pairs — pause and report progress between batches.
...
| `keep-separate` | Related but distinct — e.g. "Server Actions" vs "Server Components" are related features, not duplicates. |
...
if [ -n "$BUNDLE_GIT_ROOT" ] && [ "$BUNDLE_GIT_ROOT" = "$BUNDLE_REAL_PATH" ]; then
  if git -C "$OKF_BUNDLE_PATH" diff --quiet \
    ...
    if ! git -C "$OKF_BUNDLE_PATH" commit -m "pre-wiki-dedup snapshot" --quiet; then
      echo "Pre-write snapshot failed; abort the skill without writing any bundle files." >&2
      exit 1
    fi
    SNAPSHOT_SHA=$(git -C "$OKF_BUNDLE_PATH" rev-parse HEAD)
...
1. **More incoming links** — grep the bundle for file-relative references to the page's `node_id`; higher count wins
...
- **`sources:`** — merge both source lists (deduplicate by `resource`)
- **`relationships:`** — merge both relationship lists (deduplicate by target, prefer typed entries over untyped)
- **`base_confidence`** — recompute using the union of sources and the formula from `.skills/okf-wiki/SKILL.md`
- **`updated`** — set to now
- **`generated.by`** — set to the acting agent `<producer>/<version>`, fallback `okf-wiki/<version>`
...
### Extension-Field Preservation Checklist

When merging pages, **ALL extension fields from BOTH pages MUST be carried over** to the canonical page. This is a union merge with deduplication:

| Extension field | Merge rule |
|---|---|
| `updated` | Set to merge timestamp (overrides both) |
| `aliases` | Union of both lists, deduplicated |
| `relationships` | Union of both lists, deduplicated by `target`, prefer typed over untyped |
| `provenance` | Recompute from union of sources; keep per-source fractions per core skill merge semantics |
| `base_confidence` | Recompute using the union of sources |
| `tier` | Keep the higher tier (lower number = higher priority) |
| `lifecycle_changed` | Keep the most recent timestamp |
| `superseded_by` | Union of both lists, deduplicated |
| `status` (lifecycle) | Keep the more permissive status per the allowed-lifecycle ordering in `.skills/okf-wiki/SKILL.md` §Lifecycle |

**Lifecycle invariant:** Only a human may promote, demote, or change `status` values. The merge operation may recompute `base_confidence` and union extension fields, but any `status` change from the secondary page is **deferred to human review via `wiki-stage-commit`** when staged writes are enabled. The canonical page retains its existing `status` unless the human explicitly approves a change.

### 5c: Write a redirect stub at the secondary page path

```markdown
---
type: <secondary page type, TitleCase>
title: <secondary page title>
generated:
  by: okf-wiki/<version>
  at: <ISO timestamp now>
status: deprecated
superseded_by: <canonical node_id>
...
This page has been merged into [canonical page title](./canonical-path.md).
```

The `redirects_to:` field tells any skill reading this page to follow the redirect rather than treat it as content. The `superseded_by` field links to the canonical page using a file-relative path, enabling the lifecycle tracking system to trace page lineage.

### 5d: Rewrite links bundle-wide

Grep the entire bundle for any link pointing at the secondary slug:

- `[text](./path/to/secondary.md)` → `[text](./path/to/canonical.md)`
- `[text](../path/to/secondary.md)` → `[text](../path/to/canonical.md)`
...
**`_cache/hot.md`** — Update Recent Activity: "Merged N duplicate pairs; canonical pages updated."

### 5f: Final check

After all merges, grep the bundle for any remaining links to the secondary slug (in non-stub files). If any survive, report them — the rewrite step may have missed a non-standard link format.

## Step 6: Log

Append to `log.md`:
```
- [TIMESTAMP] DEDUP mode=audit|merge|auto-merge pages_scanned=N pairs_found=M merged=X kept_separate=Y needs_review=Z links_rewritten=W
```
...
- **`wiki-lint`** — validate that every `redirects_to:` link resolves to an existing, non-stub page (a redirect chain — stub pointing to stub — is an error)
- **`cross-linker`** — treat redirect stubs as non-targets; never add a new link pointing at a stub page
...
Derived from Ar9av/obsidian-wiki wiki-dedup skill (MIT).
